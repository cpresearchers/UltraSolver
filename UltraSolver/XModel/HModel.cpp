...
namespace cudacp {
	...
	HTab::HTab(const int id, const bool sem, vector<vector<int>>& ts, vector<HVar*>& scp, const bool transform, const bool AsPre) :
		...
		if (!AsPre) {
			//没转过
			vector<int> ori_t(arity);
			vector<int> std_t(arity);
			bool standard = true;
...
//                tuples = new int[sup_size][arity];
				tuples.resize(sup_size, vector<int>(arity));
				...
				if (!semantics) {
					...
					for (auto t : ts) {
						//                        int[] t = ts[j];
						while (!equal(ori_t.begin(), ori_t.end(), t.begin())) {
							//                            System.arraycopy(std_t, 0, tuples[j++], 0, arity);
							tuples[j++].assign(std_t.begin(), std_t.end());
							GetTuple(i++, ori_t, std_t);
						}
						GetTuple(i++, ori_t, std_t);
					}
					...
					if (!standard) {
						...
						for (auto t : ts) {
							...
							tuples[i++].assign(std_t.begin(), std_t.end());
							//                            ++i;
						}
					}
					else {
						...
						for (int i = 0; i < ts.size(); ++i) {
							...
							tuples[i].assign(ts[i].begin(), ts[i].end());
						}
					}
				}
				semantics = true;
...
//                tuples = new int[ts.length][arity];
				tuples.resize(ts.size(), vector<int>(arity));
				if (!standard) {
					//非标准表转标准表
					int i = 0;
					for (auto t : ts) {
	...
	void HTab::ToSTDTuple(vector<int> & ori_t, vector<int> & std_t) {
		for (int i = 0; i < ori_t.size(); ++i)
			std_t[i] = scope[i]->val_map[ori_t[i]];
	}
